package com.tencent.qcloud.tuikit.tuicallkit.view.callview.public.transcriber.deprecated
...
class AISubtitle(context: Context, attrs: AttributeSet?) : ScrollView(context, attrs) {
    ...
    private val trtcCloudListener = object : TRTCCloudListener() {
        override fun onRecvCustomCmdMsg(userId: String?, cmdID: Int, seq: Int, message: ByteArray?) {
            ...
            try {
                    ...
                    if (roundId.isNotEmpty()) {
                        ...
                        if (index != -1) {
                            translationInfo[index].sender =
                                if (sender.contains(Constants.AI_TRANSLATION_ROBOT)) translationInfo[index].sender else sender
                            ...
                            translationInfoItem.sender =
                                if (sender.contains(Constants.AI_TRANSLATION_ROBOT)) "" else sender
                ...
                Logger.Companion.e("translation fail, e=${e.message}")
            }
        }
    }
    ...
    private fun getUserDisplayName(userId: String): String {
        val participant = CallStore.Companion.shared.observerState.allParticipants.value.find { it.id == userId }
