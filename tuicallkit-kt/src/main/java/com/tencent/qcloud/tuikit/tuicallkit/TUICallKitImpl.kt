...
import com.tencent.qcloud.tuikit.tuicallkit.call.CallActivityRouter
import com.tencent.qcloud.tuikit.tuicallkit.call.CallBusinessBridge
...
import com.tencent.qcloud.tuikit.tuicallkit.call.IncomingCallInterceptor
...
class TUICallKitImpl private constructor(context: Context) : TUICallKit() {
    ...
    override fun calls(
        ...
        if (list.size == 1 && params?.chatGroupId.isNullOrEmpty()) {
            call(list[0] ?: "", mediaType, params, callback)
            return
        }
    ...
    override fun setCallActivityClassName(className: String?) {
        CallActivityRouter.setCallActivityClassName(className)
    }

    override fun setIncomingCallInterceptor(interceptor: IncomingCallInterceptor?) {
        CallBusinessBridge.setIncomingCallInterceptor(interceptor)
    }
    ...
    private fun tryInterceptIncomingCall(): Boolean {
        val callInfo = createIncomingCallInfo()
        val action = object : IncomingCallAction {
            override fun acceptAndOpenCallPage() {
                if (CallManager.instance.userState.selfUser.get().callStatus.get() == TUICallDefine.Status.None) {
                    Logger.w(TAG, "acceptAndOpenCallPage ignored, current status is None")
                    return
                }
                PermissionRequest.requestPermissions(
                    context, CallManager.instance.callState.mediaType.get(), object : PermissionCallback() {
                        override fun onGranted() {
                            if (CallManager.instance.userState.selfUser.get().callStatus.get()
                                == TUICallDefine.Status.None
                            ) {
                                Logger.w(TAG, "acceptAndOpenCallPage onGranted ignored, current status is None")
                                return
                            }
                            CallManager.instance.accept(object : Callback {
                                override fun onSuccess() {
                                    startCallActivityWhenAccepted()
                                }

                                override fun onError(errCode: Int, errMsg: String?) {
                                    Logger.e(TAG, "acceptAndOpenCallPage failed, errCode: $errCode, errMsg: $errMsg")
                                }
                            })
                        }

                        override fun onDenied() {
                            Logger.w(TAG, "acceptAndOpenCallPage, request Permissions failed")
            ...
            override fun rejectCall() {
                if (CallManager.instance.userState.selfUser.get().callStatus.get() == TUICallDefine.Status.None) {
                    Logger.w(TAG, "rejectCall ignored, current status is None")
                    return
                }
                CallManager.instance.reject(null)
            }
        }
        return CallBusinessBridge.interceptIncomingCall(callInfo, action)
    }

    private fun startCallActivityWhenAccepted() {
        val selfUser = CallManager.instance.userState.selfUser.get()
        if (selfUser.callStatus.get() == TUICallDefine.Status.Accept) {
            startCallActivity()
            return
        }
        lateinit var acceptedObserver: Observer<TUICallDefine.Status>
        acceptedObserver = Observer {
            if (it == TUICallDefine.Status.Accept) {
                selfUser.callStatus.removeObserver(acceptedObserver)
                startCallActivity()
            } else if (it == TUICallDefine.Status.None) {
                selfUser.callStatus.removeObserver(acceptedObserver)
            }
        }
        selfUser.callStatus.observe(acceptedObserver)
    }

    private fun createIncomingCallInfo(): IncomingCallInfo {
        var caller = CallManager.instance.userState.selfUser.get()
        val calleeIdList = ArrayList<String>()
        for (user in CallManager.instance.userState.remoteUserList.get()) {
            if (user.callRole == TUICallDefine.Role.Caller) {
                ...
                calleeIdList.add(user.id)
            }
        }
        return IncomingCallInfo(
            CallManager.instance.callState.callId,
            caller.id,
            caller.nickname.get(),
            caller.avatar.get(),
            CallManager.instance.callState.mediaType.get(),
            CallManager.instance.callState.scene.get(),
            CallManager.instance.callState.chatGroupId,
            calleeIdList
        )
    }
    ...
    private fun startCallActivity() {
        CallActivityRouter.startCallActivity(context)
    }
