...
class SingleCallVideoLayout(context: Context, private val smallVideoLayer: ViewGroup? = null) : ConstraintLayout(context) {
    ...
    private var ownsSmallRenderContainer = false
    private var smallViewTopMargin = 0
    private var smallViewEndMargin = 0
    ...
    override fun onDetachedFromWindow() {
        ...
        if (ownsSmallRenderContainer) {
            (layoutRenderSmall.parent as? ViewGroup)?.removeView(layoutRenderSmall)
        }
    }
    ...
    private fun initSmallRenderContainer() {
        val defaultSmallView: FrameLayout = findViewById(R.id.rl_video_small)
        val defaultParams = defaultSmallView.layoutParams as? ViewGroup.MarginLayoutParams
        smallViewTopMargin = defaultParams?.topMargin ?: 0
        smallViewEndMargin = defaultParams?.marginEnd ?: 0

        if (smallVideoLayer == null) {
            layoutRenderSmall = defaultSmallView
            return
        }

        defaultSmallView.visibility = GONE
        layoutRenderSmall = FrameLayout(context)
        layoutRenderSmall.visibility = GONE
        layoutRenderSmall.clipChildren = false
        layoutRenderSmall.clipToPadding = false

        val params = FrameLayout.LayoutParams(
            defaultSmallView.layoutParams.width,
            defaultSmallView.layoutParams.height,
            Gravity.TOP or Gravity.END
        )
        params.topMargin = smallViewTopMargin
        params.marginEnd = smallViewEndMargin
        smallVideoLayer.clipChildren = false
        smallVideoLayer.clipToPadding = false
        smallVideoLayer.addView(layoutRenderSmall, params)
        ownsSmallRenderContainer = true
    }
    ...
    private fun updateSmallViewSize() {
        ...
        val lp = layoutRenderSmall.layoutParams ?: createSmallViewLayoutParams()
        lp?.width = if (isLandScape) hHeight else wWidth
        lp?.height = if (isLandScape) wWidth else hHeight
        layoutRenderSmall.layoutParams = lp
    }

    private fun createSmallViewLayoutParams(): ViewGroup.LayoutParams {
        if (ownsSmallRenderContainer) {
            val params = FrameLayout.LayoutParams(
    ...
    private fun initGestureListener(view: FrameLayout) {
        val detector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onScroll(
                ...
                if (newX >= 0 && newX <= parentWidth - view.width && newY >= 0 && newY <= parentHeight - view.height) {
